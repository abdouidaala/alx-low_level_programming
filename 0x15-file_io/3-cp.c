...
int main(int argc, char *argv[])
{
	...
	if (argc != 3)
		dprintf(STDERR_FILENO, ARGUMENTS_FAILS), exit(97);
	file_from = open(argv[1], O_RDONLY);
	if (file_from == -1)
		dprintf(STDERR_FILENO, CAN_NOT_READ, argv[1]), exit(98);
	file_to = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, 0664);
	if (file_to == -1)
		dprintf(STDERR_FILENO, CAN_NOT_WRITE, argv[2]), exit(99);
	...
	if ((close_from == -1) || (close_to == -1))
		dprintf(STDERR_FILENO, CAN_NOT_CLOSE, file_from), exit(100);
	return (0);
}
